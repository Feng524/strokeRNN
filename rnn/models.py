...
def _rnn_stroke_base(x, length, len_per_stroke, scope, is_training, convolution=False, bidirectional=False,
    ...
    with variable_scope.variable_scope(
        ...
        inputs = tf.reshape(x, [-1, origin_shape[-2], origin_shape[-1]]) ##shape(batch_size, max_time, fea)
        ...
        if bidirectional:
            outputs, output_states = tf.nn.bidirectional_dynamic_rnn(cell_fw=fw_cells, cell_bw=fw_cells,
                                                                     inputs=inputs, #[None, params.length_per_signature, params.length_per_stroke, params.features]
                                                                     sequence_length=len_per_stroke, dtype=tf.float32,
                                                                     scope='bid_stroke_rnn')
            # outputs = tf.concat(outputs, axis=2)
            # output is a tuple(output_fw, output_bw)
            ...
            if is_attention:  # and 1 == 2:
                with tf.name_scope('Attention_stroke_layer'):
                    ...
                    #output shape is (batch_size, fw_output.size() + bw_output.size())
        ...
        size_embedding = STROKE_RNN_HIDDE[-1] * \
            2 if bidirectional else STROKE_RNN_HIDDE[-1] ##(双向RNN的输出是原始RNN的两倍)
...
def _loss_2logits(inputs, labels, length, len_per_stroke, stroke_features, global_features, params, is_training):
    ...
    if params.early_concat:
        length = tf.reduce_max(length, axis=1) #(None, 2) -> (None)
        ...
        if params.stroke_encoder:
            ...
            if params.global_feature: ##Encoder的结果额外拼接stroke_features
...
def model_signature(input_placeholder, mode, params):
    ...
    input_all_tower = tf.split(features, params.num_gpus, axis=0) ##shape(None, length_per_signature, length_per_stroke, features)
    ...
    for i in range(params.num_gpus):
        ...
        with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % i, merge_devices=True)):
